...
public:
  ...
  // Fast! Binary or DMA file transfer
  #if EITHER(BINARY_FILE_TRANSFER, DMA_FILE_TRANSFER)
    #if HAS_MULTI_SERIAL
  ...
  static void openFileWrite(char * const path, const uint32_t expected_filesize = 0);
  ...
  static inline int16_t write(void* buf, uint16_t nbyte) {
      int16_t out = file.isOpen()
        ? (int16_t)file.write(buf, nbyte)
        : -1;
