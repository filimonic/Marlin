...
/*   void DMAFileTransfer::save_uart_state(const uint8_t uart_port_index) {
  ...
  void DMAFileTransfer::dmaserial_start(const uint8_t uart_port_index, const uint32_t dmaft_transfer_port_speed) {
    ...
    dmaft_hdma_rx.Init.Priority            = DMA_PRIORITY_HIGH;

    HAL_DMA_Init(&dmaft_hdma_rx);

    __HAL_LINKDMA(&dmaft_huart,hdmarx,dmaft_hdma_rx);

    IRQn_Type dma_IRQn = _dmaft_stm32f1_get_dma_rx_irqn(uart_port_index);
...
/*
  ...
  void DMAFileTransfer::dmaserial_write(const uint8_t* buffer, uint16_t size, bool wait) {
    HAL_StatusTypeDef result = HAL_BUSY;
    reset_timeout();
    while(result != HAL_OK && !is_timeout_exceed()) {
      if (wait) {
        result = HAL_UART_Transmit(&dmaft_huart, (uint8_t*)buffer, size, DMAFT_TRANSFER_PORT_TIMEOUT);
      } else {
        result = HAL_UART_Transmit_IT(&dmaft_huart, (uint8_t*)buffer, size);
      }
      watchdog_refresh();
    }
  }
  ...
  void DMAFileTransfer::dmaserial_stop() {
      MYSERIAL0.printf("dmaserial_stop\n");
  }
